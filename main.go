...
func NewButtonWidget(name string, x0, y0, x1, y1 int, label string, handler func(g *gocui.Gui, v *gocui.View) error) *ButtonWidget {
	return &ButtonWidget{name: name, x0: x0, y0: y0, x1: x1, y1: y1, label: label, handler: handler}
}
...
func main() {
	...
	userBar := NewButtonWidget("userbar", 0, 0, 29, termheight-4, "Users Online", statusDown(status))
	butdown := NewButtonWidget("butdown", 30, 0, termwidth-1, termheight-4, "Chat Room", statusDown(status))
	butup := NewButtonWidget("butup", 0, termheight-3, termwidth-1, termheight-1, "Enter your message:", statusUp(status))

	g.SetManager(help, status, butdown, butup, userBar)

	if err := g.SetKeybinding("", gocui.KeyCtrlC, gocui.ModNone, quit); err != nil {
		log.Panicln(err)
	}
	if err := g.SetKeybinding("", gocui.MouseLeft, gocui.ModNone, toggleButton); err != nil {
		log.Panicln(err)
	}
